...
//     try {
//       const res = await axios.post('http://localhost:5000/api/symptom', {
//         symptoms,
//         category,
//         userId
//       })
...
//         <div className="bg-white dark:bg-slate-800/70 dark:backdrop-blur-xl border border-gray-200 dark:border-cyan-500/20 rounded-3xl shadow-sm px-6 py-4 flex items-center justify-between">
//           <div>
//             <h2 className="text-xl md:text-2xl font-semibold text-gray-900">🩺 Symptom Checker</h2>
//             <p className="text-sm text-gray-500">AI Severity Analysis</p>
//           </div>
//           <button
...
//         <div className="bg-white dark:bg-slate-800/70 dark:backdrop-blur-xl border border-gray-200 dark:border-cyan-500/20 rounded-3xl shadow-sm p-6 md:p-8">
...
//               className="w-full px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50 dark:bg-slate-700 text-gray-800 dark:text-white focus:outline-none focus:border-gray-400 transition"
//             >
...
//               rows={4}
//               className="w-full px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50 dark:bg-slate-700 text-sm text-gray-800 dark:text-white placeholder-gray-400 dark:placeholder-gray-300 focus:outline-none focus:border-gray-400 transition resize-none"
//             />
//           </div>
...
//             className="w-full bg-black hover:bg-gray-800 active:scale-[0.98] disabled:opacity-50 transition-all text-white font-medium py-3.5 rounded-2xl text-sm"
//           >
...
//         {result && (
//           <div className="mt-4 bg-white dark:bg-slate-800/70 dark:backdrop-blur-xl border border-gray-200 dark:border-cyan-500/20 rounded-3xl shadow-sm overflow-hidden">
...
//               <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">AI Advice</p>
...
function Symptom() {
  ...
  const checkSymptoms = async () => {
    ...
    const userId = localStorage.getItem('userId') || 1
    try {
      const res = await axios.post('http://localhost:5000/api/symptom', { symptoms, category, userId })
      setResult(res.data)
    } catch (err) {
      setResult({ severity: 'ERROR', advice: 'Something went wrong. Please try again.', color: 'gray' })
    }
